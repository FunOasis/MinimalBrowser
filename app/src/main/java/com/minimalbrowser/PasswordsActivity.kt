...
class PasswordsActivity : AppCompatActivity() {
    ...
    private fun loadPasswords() {
        ...
        passwords.forEach { p ->
            val row = LayoutInflater.from(this)
                .inflate(android.R.layout.simple_list_item_2, binding.listPasswords, false)
    ...
    private fun showAddDialog() {
        ...
        val etDomain   = EditText(this).apply { hint = "Domain (e.g. google.com)"; setTextColor(0xFFFFFFFF.toInt()) }
        val etUsername = EditText(this).apply { hint = "Username / Email"; setTextColor(0xFFFFFFFF.toInt()) }
        val etPassword = EditText(this).apply { hint = "Password"; inputType = android.text.InputType.TYPE_CLASS_TEXT or android.text.InputType.TYPE_TEXT_VARIATION_PASSWORD; setTextColor(0xFFFFFFFF.toInt()) }
        ...
        when (requestCode) {
            EXPORT_REQUEST -> {
                contentResolver.openOutputStream(uri)?.let { stream ->
                    pm.exportCsv(stream)
                    stream.close()
                    Toast.makeText(this, "Passwords exported", Toast.LENGTH_SHORT).show()
                }
            }
            IMPORT_REQUEST -> {
